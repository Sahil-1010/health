...
let booking_status=0;//1 for patient 2 for register;
let show_other=document.querySelector('.show_other');
const beds = document.querySelector('.beds'); // Select the .beds section

function toggleBedAvailability(hospitalId) {
    const bedSection = document.getElementById(hospitalId);
    if (bedSection.style.display === 'flex') {
      bedSection.style.display = 'none';
    } else {
      bedSection.style.display = 'flex';
    }
  }

function markRandomBedsAvailable() {
    const beds = document.querySelectorAll('.bed');
    const totalBeds = beds.length;

    // Randomly pick a number between 1 and 6 for available beds
    const availableCount = Math.floor(Math.random() * 6) + 1;

    // Create a shuffled array of indexes to randomly choose beds
    const bedIndexes = Array.from({length: totalBeds}, (_, index) => index);
    bedIndexes.sort(() => Math.random() - 0.5);

    // Mark random beds as available
    for (let i = 0; i < availableCount; i++) {
      beds[bedIndexes[i]].classList.add('available');
      beds[bedIndexes[i]].innerText += ' - Available';
    }
  }

  // Run the function when the DOM content is loaded
  document.addEventListener('DOMContentLoaded', markRandomBedsAvailable);

document.addEventListener('DOMContentLoaded', function() {
    // Fade-in effect for headings and paragraphs
    const headings = document.querySelectorAll('h1, h2, p, h5');
    headings.forEach(heading => {
      heading.style.opacity = 1; // Change opacity to make them visible
    });
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          img.style.opacity = 1; // Change opacity to make image visible
          observer.unobserve(img); // Stop observing after image is visible
        }
      });
    }, { threshold: 0.1 }); // Adjust the threshold as needed

    images.forEach(img => {
      img.style.opacity = 0; // Start with images invisible
...
function check_data(){
    ...
    run_data();
    show_other.style.display='block';
    queue=2;
    }
    else{
...
function run_data() {
    ...
    const queuePositionElement = document.getElementById('queue-position');
    const nextMessage = document.getElementById('next-message');
    const appointedUsers = document.querySelectorAll('.show_other li');
    const audio = new Audio('audio-file-path.mp3'); // Add your audio file path
    ...
    const queueInterval = setInterval(() => {
        if (queuePosition <= 10) {
            ...
            // Loop through the appointed users list and highlight the matched user
            appointedUsers.forEach((user, index) => {
                if (index + 1 === randomToken) {
                    user.style.backgroundColor = '#6edc6e99';
                    // Highlight matching user in green
                } else {
                    user.style.color = '';  // Reset color for non-matching users
                }
            });
            ...
            if (queuePosition === randomToken - 1) {
                ...
                // Play audio when token matches queue position
                audio.play();
            } else {
